...
func (c *Client) GetPokemonInfo(req_url string) (PokemonInfo, error) {
	cachedData, exists := c.cache.Get(req_url)
	var pokemonInfo PokemonInfo
	if exists {
		if err := json.Unmarshal(cachedData, &pokemonInfo); err != nil {
			return PokemonInfo{}, fmt.Errorf("error unmarshelling pokemon info")
		}
		return pokemonInfo, nil
	}
	res, err := c.client.Get(req_url)
	if err != nil {
		return PokemonInfo{}, fmt.Errorf("error listing pokemon info")
	}
	defer res.Body.Close()

	body, err := io.ReadAll(res.Body)
	if err != nil {
		return PokemonInfo{}, fmt.Errorf("error reading body")
	}
	c.cache.Add(req_url, body)
